...
*/
function inicializaImporteArea()
{
	if(logDebugImporteArea)
	{
		console.log("ImporteArea");
	}
...
function inicializaDatosImporteArea()
{
	...
	insertaURLSAPI();
	var texto = $.i18n( 'importe_area' );
	$('#textoCabeceraImporteArea').html(texto+' '+anyo);
	
	var importeAreas=new Array();
	...
	var jqxhr = $.getJSON(dameURL(queryGraficoDepGasto+urlFiltroAnyo)).done(function( data ) 
	{
		if ((data!=null)&&(data.records!=null)&&(data.records.length>0))
		{
			...
			for (var i = 0; i < data.records.length; i++) 
			{
				var title = organizaciones[data.records[i].area];
				areas.push(title);
				var importeArea=new Object();
				importeArea.name=title.substring(0, limiteCadenasTexto);
				importeArea.nameCompl=title;
				importeArea.value=data.records[i].suma;
				importeAreas.push(importeArea);
	...
	{
		pintaGrafico(importeAreas);			
		modificaTaskMaster("iframeImporteArea");
	});
}
...
function insertaURLSAPI()
{
	...
	$('#urlAPIDepGasGlo').attr("href", queryGraficoDepGasto+urlFiltroAnyo);
	$('#descargaDepGasGloCSV').attr("href", queryGraficoDepGastoCSV+urlFiltroAnyo);
	$('#descargaDepGasGloJSON').attr("href", queryGraficoDepGasto+urlFiltroAnyo);
...
*/
function pintaGrafico(importeAreas)
{
	if(logDebugImporteArea)
	{
		console.log("pintaTarta");
	}
	...
	series.slices.template.events.on("hit", function(ev) 
	{
		...
		).always(function() 
		{
			if(window.self!=window.top)
			{
				...
				window.parent.cambioCapaSubvencionesAreaNombre(paramOrgId,anyo);
			}else
			{
				console.log('Ejecutar enlace');
				var url = "busqueda_convocatorias.html?lang="+$.i18n().locale;
				url=url+"&areaNombre="+paramOrgId+"&anyo="+anyo;
...
function mostrarDatos() 
{
	...
	if (isVisible === true) 
	{
		...
		$('#iframeImporteArea', window.parent.document).height($( document ).height()+cachito);
	}else
	{
		$("#iframeImporteArea", window.parent.document).height(394+120);
	}
}